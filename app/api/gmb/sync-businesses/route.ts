...
export async function POST(request: NextRequest) {
  try {
    ...
    if (googleAccount.last_businesses_sync_at) {
      const lastSyncTime = new Date(googleAccount.last_businesses_sync_at).getTime();
      const now = Date.now();
      ...
      if (timeSinceLastSync < COOLDOWN_MS) {
        ...
        return NextResponse.json(
          {
            error: 'sync_cooldown',
            message: `Please wait ${remainingMinutes} minute(s) before syncing again`,
            retryAfter: remainingMs,
            remainingSeconds,
            lastSyncedAt: googleAccount.last_businesses_sync_at,
          },
          { status: 429 }
        );
      }
    ...
    await supabase
      .from('google_accounts')
      .update({
        last_businesses_sync_at: new Date().toISOString(),
        businesses_sync_count: (googleAccount.businesses_sync_count || 0) + 1,
        updated_at: new Date().toISOString(),
      })
